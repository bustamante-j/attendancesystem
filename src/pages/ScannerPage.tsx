    ...
    <div className="mx-auto max-w-6xl space-y-5">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h1 className="text-2xl font-bold">{eventRecord.name}</h1>
          <p className="mt-1 text-sm text-slate-500">{eventRecord.venue || 'No venue'} · {formatManilaDate(eventRecord.start_at)}</p>
        </div>
          ...
          <section className="panel p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="inline-flex rounded-lg border border-slate-300 bg-slate-100 p-1">
                <button className={`rounded-md px-4 py-2 text-sm font-medium ${direction === 'check_in' ? 'bg-white text-blue-800 shadow-sm' : 'text-slate-600'}`} onClick={() => setDirection('check_in')}>Check in</button>
                {eventRecord.attendance_mode === 'check_in_out' && <button className={`rounded-md px-4 py-2 text-sm font-medium ${direction === 'check_out' ? 'bg-white text-blue-800 shadow-sm' : 'text-slate-600'}`} onClick={() => setDirection('check_out')}>Check out</button>}
              </div>
            </div>
          </section>
          ...
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1.5fr)_minmax(300px,0.8fr)]">
            <section className="panel p-3 sm:p-5">
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                ...
                {cameraActive
                  ? <button className="btn-secondary" onClick={() => setCameraActive(false)}><CameraOff size={17} /> Stop camera</button>
                  : <button className="btn-primary" onClick={() => void startCamera()}><Camera size={17} /> Start camera</button>}
              </div>
            </section>

            <div className="space-y-5">
              <section className="panel min-h-40" aria-live="assertive" aria-atomic="true">
                {!result || !currentTone ? (
