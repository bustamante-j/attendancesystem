import { CalendarX2, ClipboardList, Copy, Eye, History, KeyRound, Pencil, Plus, ScanLine, Trash2, UserPlus } from 'lucide-react'
...
export function EventsPage() {
  ...
  return <div className="space-y-5">
    ...
    <div className="table-wrap"><table><thead><tr><th>Event</th><th>Audience</th><th>Schedule</th><th>Mode</th><th>Status</th><th>Attendance progress</th><th>Attendance</th>{canManage && <th>Actions</th>}</tr></thead><tbody>
      {visibleEvents.map((event) => {
        ...
        return <tr key={event.id}>
          ...
          <td><div className="flex flex-wrap gap-2"><button className="btn-secondary" onClick={() => navigate(`/events/${event.id}/attendance`)}><ClipboardList size={15} /> Roster</button>{event.is_historical ? <span className="self-center text-xs font-medium text-slate-500">Completed record</span> : <button className="btn-primary" disabled={event.status !== 'open'} onClick={() => navigate(`/events/${event.id}/scanner`)}><ScanLine size={15} /> Scan</button>}</div></td>
