...
export function EventsPage() {
  ...
  return <div className="space-y-5">
    ...
    <div className="table-wrap"><table><thead><tr><th>Event</th><th>Audience</th><th>Schedule</th><th>Mode</th><th>Status</th><th>Expected</th>{canManage && <th>Actions</th>}</tr></thead><tbody>{visibleEvents.map((event) => { const audience = audiences[event.id]; return <tr key={event.id}><td><div className="font-medium">{event.name}</div><div className="text-xs text-slate-500">{event.venue || 'No venue'}</div></td><td><div className="flex flex-wrap gap-1">{audience?.departmentIds.map((id) => <span key={id} className="rounded bg-slate-100 px-2 py-1 text-xs">{departmentMap.get(id) ?? 'Unknown'}</span>)}</div><div className="mt-1 text-xs text-slate-500">{audience?.yearLevels.length ? `Years ${audience.yearLevels.join(', ')}` : 'All year levels'}</div></td><td>{formatManilaDate(event.start_at)}<div className="text-xs text-slate-500">to {formatManilaDate(event.end_at)}</div></td><td>{event.attendance_mode === 'check_in_out' ? 'Check-in/out' : 'Check-in only'}</td><td><span className={`rounded-full px-2 py-1 text-xs capitalize ${event.status === 'open' ? 'bg-emerald-100 text-emerald-800' : event.status === 'closed' ? 'bg-slate-200 text-slate-700' : 'bg-amber-100 text-amber-800'}`}>{event.status}</span></td><td>{counts[event.id] ?? 0}</td>{canManage && <td><div className="flex flex-wrap gap-2"><button className="btn-secondary" onClick={() => openEdit(event)}><Pencil size={14} /> Edit</button><button className="btn-secondary" onClick={() => void resetPin(event)}><KeyRound size={14} /> Reset PIN</button>{event.status !== 'open' && <button className="btn-secondary" onClick={() => void changeStatus(event, 'open')}>Open</button>}{event.status !== 'closed' && <button className="btn-secondary" onClick={() => void changeStatus(event, 'closed')}>Close</button>}<button className="btn-danger" onClick={() => void remove(event)}><Trash2 size={14} /></button></div></td>}</tr>})}{!visibleEvents.length && <tr><td colSpan={canManage ? 7 : 6} className="py-10 text-center text-slate-500">No events match the filters.</td></tr>}</tbody></table></div>
