  ...
  return (
    <Modal title={mode === 'viewed' ? 'View student QR credential' : credentials.length > 1 ? `Issued ${credentials.length} QR credentials` : 'QR credential issued'} onClose={onClose} size={isSingleCredential ? 'md' : 'full'}>
      <div className={isSingleCredential ? 'mx-auto max-w-md' : ''}>
        <div className="no-print mb-5 rounded-xl border border-blue-200 bg-blue-50 p-4 text-center text-sm text-blue-900 dark:border-blue-800 dark:bg-blue-950/50 dark:text-blue-200">
        ...
        <div className={`no-print mb-5 flex flex-wrap items-center gap-3 ${isSingleCredential ? 'justify-center text-center' : ''}`}>
        ...
        <div className={`qr-print-area grid gap-5 ${isSingleCredential ? 'mx-auto max-w-sm grid-cols-1' : 'sm:grid-cols-2 lg:grid-cols-3 print:grid-cols-2'}`}>
